...
for x in hands_bids:
    ...
    if ' ' in hand:
        new_list = list(filter(lambda x: x != ' ', hand))
        print(new_list)
        if len(new_list) == 0:
            new_list = ['0', '0', '0', '0', '0']
        common = (max(set(new_list), key=new_list.count))
        joker = list(map(lambda x: x.replace(' ', common), hand))
        x.update([('joker', joker)])
    
    newhand = list(map(lambda x: x.replace(' ', '0'), hand))
    x.update([('hand', newhand)])
